...
/*   Updated: 2019/04/17 17:15:43 by zmahomed         ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
...
{
	clock_t begin = clock();//For runtime clock counter (i think its a clock counter)
...
//			printf("\t\t%i\t%i\n",line_cur,char_count); //Simple debug for line by line tracing
	...
	//sleep(1);
	clock_t end = clock();
	print_stats(err_tot, line_cur, (double)(end - begin));
}

//Printing final stats
void	print_stats(int errors, int lines, double time_tot)
{
	printf("\033[33;1mRuntime: %.2lf",time_tot);
	printf("\n\033[33;1mLines: %i",lines);
